# HORIZON SHIELD 引き継ぎ 2026-09-05 深夜 v35 (見積もり鑑定書AI・門 v1.3.3・社名 789 本・JIDEC entry 34・SSRN 共著確定)

**このファイル 1 つで、止まったチャットの続きから動ける。**

作成: 2026 年 9 月 5 日 23:58 JST(v34 = 23:40 に、23:34 以降の共著確定・論文 v0.2・返信送信・A2A の見立てを組み込んだ完全版。**v34 は読まなくてよい、本ファイルが正**)
作成者: 番人(このセッションの本人。Cowork、Mac にリンク済み、horizon-shield / jidec / mcp-conduct-register の各フォルダにアクセス済み)
引き継ぎ元: 「停止した問題の引き継ぎ」チャット(2026-09-05 13:00〜23:58 JST、途中 1 回コンテキスト圧縮。前半は v33 の 20〜24 章、本ファイルは 24 章の続きから)
...
7. SSRN working paper 1 本目を書き始めた。**Federico が共著を受けた(23:34)**、論文は v0.2(著者 2 名、分担明記、4.2 節と 6 節は彼が書く)、commit 8e2c061a で公開リポジトリに(9 章)。
8. LinkedIn 投稿の文面を用意(Sánchez 版)。**投稿したかは未確認**(8 章)。
9. 「A2A に完璧に組み込めばインフラになるか」への番人の答え: 半分正しい、決めるのは採用、島にならん道は SCITT 互換(16 章)。
...
- TOshi 送信済(23:34 より前、スクショで確認): `ops/fed_reply_ssrn_invite_20260905.txt`(403 語。1 行目 Sánchez の件、23:03 への返事、SSRN 共著の誘い三択: 共著 / 名前付き独立検証者として草稿を確認 / 記録引用のみ)。
- **23:34 彼: 共著を受けた。** 所属は本人の文言「Viper Labs (builds invinoveritas, a verification layer for autonomous agents)」。再実装の節は自分で書く、草稿は用意でき次第送ってくれ、bytes first の順で正しい、9 月の blind 再実行は合意どおり(食い違えば均さず報告)。
- TOshi 送信済(23:50 頃、本人申告「送ったぞ」): `ops/fed_reply_coauthor_20260905.txt`(265 語): 所属受領、草稿は添付やなく **リポジトリの path と commit 8e2c061a**、彼の担当は 4.2 節・6 節・Table 3、7 節 limits にも赤を、変更は branch / diff / 本文どれでも、**SSRN の共著者登録に彼のメールアドレスが要るので DM で送ってくれ(リポジトリには書かない)**、期限なし。
- **彼の次の返事(メールアドレス、4.2 節・6 節の原稿)は未着。** LinkedIn 投稿(`ops/linkedin_post_reimpl_20260905.txt`、Sánchez 版)を出したかは **未確認**(23:07 時点で下書き状態やった)。
...
### 9.1 1 本目(共著で進行中)
- `papers/nenrin-reproducibility/manuscript_v0.1.md`(ファイル名はリンク安定のため据え置き、中の版表記で管理。**現在 v0.2、3,549 語、commit 8e2c061a、push 済、公開リポジトリで読める**)。
...
- **著者 2 名**: Toshikatsu Oga(The HORIZ音s株式会社 / HORIZON SHIELD、ORCID 0009-0000-9180-903X)、Federico Blanco Sánchez-Llanos(Viper Labs (builds invinoveritas, a verification layer for autonomous agents))。著者欄の下に分担: T.O. = NENRIN の設計・運用・参照実装・草稿、F.B.S.L. = Node.js 実装と blind 実行、**4.2 節と 6 節は彼が書く(今の文は entry 34 から書いた仮置き、頭に印あり)**、最終稿は二人で承諾。謝辞は「両著者とも報酬なし」+ 利益相反(著者が全 endpoint と台帳を運営)。私信 DM の引用は無し。
...
- 構成: Abstract / 1 問い / 2 背景(discovery without conduct、先行研究 CT・Rekor・Reproducible Builds・in-toto・SLSA・OTS・RFC 8785)/ 3 Layer 3 as implemented(3.1 入力と 30 件天井の失敗談、3.2 出力、3.3 バイト、3.4 錨)/ 4 手順(4.1 recompute、4.2 独立再実装 4 条件【彼】、4.3 記録)/ 5 結果(Table 1 リング 8 本、Table 2 artifact)/ 6 seam(Table 3)【彼】/ 7 limits(決定性 ≠ 真実、n=2、入力は運営者公開、正準形はランタイム定義、保持の失敗)/ 8 事前登録 / 9 結論 / 付録 A 再現手順 / 付録 B entry 34 の record(**未記入**)/ 謝辞と利益相反 / 参考文献(**未整形**)。
- **確定した流れ**: 彼が 4.2 節・6 節(と Table 3 の訂正、7 節への赤)を branch / diff / 本文で送る → 番人が merge して v0.3 → 番人が参考文献と付録 B(entry 34 の raw record、Bitcoin ブロックが付いてから)と abstract の最終調整 → 二人で最終稿承諾 → TOshi が SSRN に投稿(共著者は **彼のメールアドレス**で登録、SSRN が本人に確認を飛ばす。メールは彼が DM で送る約束)。
- **TOshi だけが判断できる 5 箇所**(明日、頭のはっきりしとる時): 3.1 の保持の話(30 → 400、一掃分で消えるところやった)の語り、Table 1 gate の held 9 の理由(同じエッジ網の内側から自分の戸口に届かん = error 1104)、p001 pending 26 の行を載せるか(商売の側)、7 節「運営者の export から運営者のリングを再現するのは builder の試験であって運営者の試験ではない」を残すか(番人は残す派)、謝辞の利益相反の一文。
- 番人が今日直した自分の誤り: 6 節に「再実装は最初の試みでは成功しなかった」と書いたが、彼はそう言うてへん(「推測してたら壊れてた、確かめたから壊れなかった」)。削除して彼の趣旨に合わせた。**論文でも DM でも、彼の言葉を超えて書かない。**
...
9. A2A の extension(conduct-v1 の URI と 1 枚の spec)を作るか、Federico の invinoveritas を登録簿の最初の外部行にする DM を出すか、Smithery への 1 通を出すか(16 章)。

---

## 14. 今日作った・変えたファイル(横断一覧)

**horizon-shield(全部 push 済、最新 fbc46e22)**
...
- commit 済(26f8176a): 引き継ぎ v34、`papers/nenrin-reproducibility/manuscript_v0.1.md`、`ops/fed_reply_ethglobal / reimpl_final / ssrn_invite`、`ops/linkedin_post_reimpl_20260905.txt`、`ops/register_recompute_note` / `register_reimpl_note`、`ops/pagecheck.yml.v132` / `.v133`。commit 済(8e2c061a): 論文 v0.2、`ops/fed_reply_coauthor_20260905.txt`。commit 済(fbc46e22): v34 の 17 章追記、coauthor 返信への commit 差し込み。
- **意図して commit していない**: `ops/fed_reply_reimpl_20260905.txt`(272 語の旧版、使わない)。**本ファイル(v35)は未 commit。**

**mcp-conduct-register(push 済、最新 be89099)**: README に「Independent recompute (record)」の段落 2 本(c153060 → a6a709e)、Sánchez 修正(be89099)。

**貼る物(TOshi)**: LinkedIn 投稿 `ops/linkedin_post_reimpl_20260905.txt`(274 語、Sánchez 版)だけが **未確認**。DM 2 通(ssrn_invite、coauthor)は送信済。
...
- **Viper Labs**: Federico の所属(本人の文言)。invinoveritas = 彼の verification layer for autonomous agents、公式 MCP レジストリにも server あり。
- **SCITT**: IETF Supply Chain Integrity, Transparency and Trust。署名付き声明の透明性サービスの標準。16 章。
...
## 16. 「A2A に完璧に組み込めばインフラになるか」への番人の答え(23:52 JST、TOshi の余談への返答。決裁事項ではない)

**半分正しい。「組み込めば」は正しい。「なる」は設計では決まらん、採用で決まる。** インフラとは、他人の判断がこの記録に依存しとる状態のこと。

正しい側: A2A の agent card は conduct 記録への指し先を置く場所そのもの(扉は既に条件 2・3 で card を読み、台帳は自分の card を出しとる)。効く形は 3 つ。(1) card の `extensions` に conduct-v1 の URI を置いてリングの URL と台帳エントリを指す(A2A の extension は許可制やない、URI と 1 枚の spec で今日でも出せる)。(2) agent が別の agent に仕事を投げる前の「接続前の行儀確認」(09-04 の結論どおり、今日の時点で AI がバッジを探す唯一の実需、先客なし)。(3) **A2A で繋いだ client を全員 証人にする**(接続ごとに walk が台帳に落ちれば、証人が使用量に比例して増え、不一致検出がネットワーク規模で回る。8 月のリングの「証人 1 人」問題が構造として消える)。

なる側の条件 3 つ、今は全部ゼロ: 登録簿の自社以外の行(0)、一覧の横にバッジを並べるレジストリ(0 社)、接続前に記録を引く agent フレームワーク(0)。動かすのは Federico の invinoveritas を最初の外部行にする DM 1 通と、Smithery への 1 通。

警告: **島になる危険。** MCP/A2A の本家、IETF SCITT(署名付き声明の透明性サービス)、Sigstore Rekor、agent 身元の勢力が「信頼 / attestation」の欄を自分で定義した時、一人の運営者の独自形式は迂回される。生き残る道は「全員が JIDEC に来る」やなく、**リングと witness 記録を SCITT の receipt / in-toto の attestation として出せる形にして向こうの土管に乗る**。堀は形式やなく時間(何か月分の再計算できるリング、自己適用、Bitcoin の刻印)。形式は真似される、時間は買えん。

金: インフラ自体は稼がん。払うのは agent を繋ぐ前に行儀を知りたい買い手側(調達・保険・企業の agent 運用)= HS の買い手側の思想と同じ向き。レジストリから取ると中立が崩れる。

順番(番人案): extension の URI と 1 枚の spec(1 日)→ Federico の行を登録簿に(DM 1 通)→ Smithery(DM 1 通)→ SCITT 互換の出力(設計 1 週間)。「完璧に組み込む」は最後。
...
2. TOshi に聞くのは **3 つだけ**: (a) LinkedIn 投稿は出したか、Federico から共著返信への返事(SSRN 用メールアドレス、4.2 節・6 節の原稿)は来たか、(b) pagecheck の 642464c1 / 39c06320 のメールは緑か、(c) entry 35 は 09:30 JST に入ったか。**それ以外の「送ったか」は聞かない**(SSRN 誘いと共著返信の 2 通は送信済み、確認済み)。
3. 明朝の手順(12 章)の結果を受け取る。
4. Federico の 4.2 節・6 節が来とれば merge して v0.3。来てなくても番人は参考文献と付録 B(entry 34 の raw、Bitcoin ブロックが付いてから)を仕上げる。彼のメールアドレスは DM で来る、リポジトリに書かない。
