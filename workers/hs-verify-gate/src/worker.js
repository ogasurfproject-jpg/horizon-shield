...
function gateDisclaimers(record) {
  ...
  if (sig && typeof sig === "object") {
    ...
    if (sig.present === 0) {
      ...
      est.push("the agent card was signed with the key at " + (ownGood.jku || "an unnamed key location") + " (kid " + (ownGood.kid || "none") + "), which is served under the agent's own domain, and the signature verifies over the canonical card, so what the card declares is attributable to the operator and not only to this gate's observation");
    } else if (ownBad) {
      dne.push("that the operator published this card: a signature whose key is served under the agent's own domain (kid " + (ownBad.kid || "none") + ") does not verify (" + (ownBad.reason || "no reason recorded") + ")" + (foreignGood ? ", and the signature that does verify uses a key at " + hostOf(foreignGood.jku) + ", which is not under the agent's own domain" : "") + ", so these bytes remain attributable to this gate's observation alone");
    } else if (foreignGood) {
      dne.push("that the operator published this card: the signature verifies, but against a key served at " + hostOf(foreignGood.jku) + ", which is not under the agent's own domain. Attribution would then rest on a key server the operator need not control, so the operator can disown it and this gate does not claim it (an off-domain key is permitted and is not a failure)");
    } else {
