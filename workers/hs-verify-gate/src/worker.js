...
// 0.4.15 (2026-09-25). 法人身元を署名の内側へ。A2A 1.0 §8.4 の正規化は card を proto の形に写してから RFC 8785 を掛ける。
// proto の AgentProvider には organization と url しか欄が無いので、provider.legalEntity は署名の前に落ちる
// (@a2a-js/sdk 1.1.0 canonicalizeAgentCard で実測、法人番号は canonical bytes に 1 度も出ん)。仕様準拠の 1 本目の署名は
// 法人番号を覆っとらんかった。extension の params は正規化を生き残るので、同じ値を legal-entity-v1 の params にも置く。
// 出所は下の 1 つだけ。provider.legalEntity と params は同じ定数から描くので、一致は構造で担保される(試験も縛る)。
...
// 出る。読む側がそれを知らんかったら、本家の推奨に従った奴が黙って条件3 で落ちる。
// 識別子は今も 1 本 = CONDUCT_EXT_URI。下は「読む時に認める綴り」で、閉じた一覧・完全一致のみ。
// 正規化せん(大文字小文字・末尾スラッシュ・http・他の path・他の版は全部 別物)。
// check の最中に redirect は絶対に叩かん(他所の DNS にこの拡張の正体を決めさせんため、
// それと掃引の subrequest 予算のため)。どっちの綴りで宣言されたかは判定のバイトに書く。
const CONDUCT_EXT_PERMANENT_ID = "https://w3id.org/horizonshield/conduct/v1";
const CONDUCT_EXT_NEXT_VERSION_ID = "https://w3id.org/horizonshield/conduct/v2";
const CONDUCT_EXT_URIS = [CONDUCT_EXT_URI, CONDUCT_EXT_PERMANENT_ID];
function conductExtUriForm(u) { return u === CONDUCT_EXT_URI ? "canonical" : u === CONDUCT_EXT_PERMANENT_ID ? "permanent_id" : null; }
...
const CARD_SIGNATURE = {
  ...
  "signature": "_TJ27IJZzERGXlz5oRsipaovjQS9naVfBrKXMP5AQ901cN2hwvSnOU0XVm1xSvfIUIUBNH5t_sLvkSuNjIZfwg",
  "plain": {
    "protected": "eyJhbGciOiJFUzI1NiIsInR5cCI6IkpPU0UiLCJraWQiOiJocy0yMDI2LTA5Iiwiamt1IjoiaHR0cHM6Ly9nYXRlLmhvcml6b25zaGllbGQuZGV2Ly53ZWxsLWtub3duL2p3a3MuanNvbiJ9",
    "signature": "_rqucMoguUKlDB1TVxqOR4leUbHMDfzL7TD5Tl8ykI75co4e8I8ejekAh5aLwpATiPywYovFNsnGCU0dZpJWYw"
  },
  "jwk": {
    "kty": "EC",
  ...
  "canonical_sha256": "1489fe91a2d6cbd6fcc88ce40d3c929eea9597832c3044fdc2c68877907d9f75",
  "jcs_sha256": "d9fe9289b493f93e2d771a0fcda118cf39177baf98392a2ad299f930098aef1a"
};
/* @@CARD_SIGNATURE_END */
...
function withCardSignature(card, origin) {
  ...
  // 0.4.15. 並びは plain(同じ鍵で素の RFC 8785 に署名した写し、url を含む配信バイト全部を覆う)を先頭、公式 SDK 流を 2 本目に。
  // 公式 SDK の verifier は「どれか 1 本が通れば可」で並びに依らず通る(実測: verify_live_card を並べ替えた card で VERIFIED)。
  // 一方、先頭しか見ん素朴な検証器は先頭が SDK 流やと落ちる(Agenstry が該当と推定: 全署名を素の JCS で見るなら plain で通るはずが 0/10)。
  // 先頭を素の JCS にすれば、両方の流儀の検証器で通る。署名の中身は 1 バイトも変わらん(並びは署名対象の外)。
  const entries = [];
  ...
  entries.push({ protected: CARD_SIGNATURE.protected, signature: CARD_SIGNATURE.signature });
...
// 0.4.11. did:web:gate.horizonshield.dev を配る = registry に依らん識別子。DID は https://gate.horizonshield.dev/.well-known/did.json に解決する。
// 載せるのは card 署名鍵 (kid hs-2026-09、card の JWS を作る鍵) と、在れば運営者鍵 (TSUGI の許可を署名する Ed25519)。両方この扉が既に公開しとる公開鍵で、秘密鍵は Worker に無い。
// id は正規ドメインで固定 (workers.dev の別名で開いても DID はこのドメインに縛られる。card 署名と同じ規律)。運営者鍵は env の raw base64、DID の JWK は base64url なので直す。
function operatorJwk(env) {
  const b = (env && env.OPERATOR_PUBKEY_B64 ? String(env.OPERATOR_PUBKEY_B64) : "").trim();
  if (!b) return null;
  const x = b.replace(/\s+/g, "").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  return { kty: "OKP", crv: "Ed25519", x, kid: "operator", use: "sig" };
}
function didDocument(env) {
  const host = CARD_CANONICAL_ORIGIN.replace(/^https:\/\//, "");
  const did = "did:web:" + host;
  const vm = [], refs = [];
  if (CARD_SIGNATURE && CARD_SIGNATURE.jwk) {
    const id = did + "#" + (CARD_SIGNATURE.kid || "card");
    vm.push({ id, type: "JsonWebKey2020", controller: did, publicKeyJwk: CARD_SIGNATURE.jwk });
    refs.push(id);
  }
  const opk = operatorJwk(env);
  if (opk) vm.push({ id: did + "#operator", type: "JsonWebKey2020", controller: did, publicKeyJwk: opk });
  return {
    ...
    authentication: refs,
    service: [
      { id: did + "#a2a", type: "A2A", serviceEndpoint: CARD_CANONICAL_ORIGIN + "/a2a" },
      { id: did + "#mcp", type: "MCP", serviceEndpoint: CARD_CANONICAL_ORIGIN + "/mcp" },
      { id: did + "#agent-card", type: "AgentCard", serviceEndpoint: CARD_CANONICAL_ORIGIN + "/.well-known/agent-card.json" }
    ]
  };
}
...
function conductExtDeclarations(card) {
  ...
  return exts.filter((e) => e && typeof e === "object" && !Array.isArray(e) && CONDUCT_EXT_URIS.includes(e.uri));
}
...
// 0.4.15. legal-entity-v1 の宣言 1 本。data-only(活性化も echo も無い)。params は LEGAL_ENTITY + 照会先。
function legalEntityExtension() {
  return {
    uri: LEGAL_ENTITY_EXT_URI,
    description: "The legal entity that answers for this agent, declared inside the signed bytes. The specification is served at the URI.",
    required: false,
    params: Object.assign({}, LEGAL_ENTITY, { lookup_url: LEGAL_ENTITY_LOOKUP_URL })
  };
}
async function legalEntityExtResponse(request) {
  const accept = request.headers.get("Accept") || "";
  if (/text\/markdown/i.test(accept)) {
    return new Response(LEGAL_ENTITY_EXT_MD, { headers: { "Content-Type": "text/markdown; charset=utf-8", "Cache-Control": "public, max-age=3600", ...CORS_HEADERS } });
  }
  return json({
    uri: LEGAL_ENTITY_EXT_URI,
    name: "A2A Legal Entity Extension",
    version: "v1",
    since_gate: "0.4.15",
    type: "data-only extension on the A2A Agent Card; no activation, no request-level echo",
    required: false,
    spec_markdown_sha256: await sha256hex(LEGAL_ENTITY_EXT_MD),
    why: "A2A 1.0 section 8.4 signs the proto-shaped card. The proto AgentProvider has organization and url only, so provider.legalEntity is dropped before signing. Extension params survive canonicalization, so an identifier declared here sits inside the bytes a spec-conformant signature covers.",
    declaration: {
      where: "capabilities.extensions[] with uri equal to this URI, required false",
      params: {
        registry: { requirement: "REQUIRED", type: "string", meaning: "ISO 3166-1 alpha-2 code of the issuing jurisdiction, or GLEIF for an LEI" },
        scheme: { requirement: "REQUIRED", type: "string", meaning: "houjin-bango (Japan, 13 digits), lei (ISO 17442, 20 characters), or the scheme name the register uses" },
        id: { requirement: "REQUIRED", type: "string", meaning: "the identifier exactly as the register publishes it" },
        name: { requirement: "REQUIRED", type: "string", meaning: "the name as the agent declares it; not asserted to match the register" },
        lookup_url: { requirement: "OPTIONAL", type: "https URL", meaning: "a page at the register where anyone can look the identifier up" }
      }
    },
    mirror_rule: "When the card also carries provider.legalEntity, registry, scheme, id and name MUST equal the params. A reader that finds them unequal records a discrepancy and relies on neither.",
    establishes: ["the holder of the card signing key declared this identifier inside the signed bytes when it signed (when the signature verifies under a key served at a jku on the agent's own host)"],
    does_not_establish: ["that the register lists the identifier", "that the declared name matches the register", "that the entity controls the domain", "anything about conduct or prices"],
    example: legalEntityExtension(),
    specification_markdown: "Send Accept: text/markdown to this URL for the full text"
  });
}
function conductExtensionSpec(origin, mdSha) {
  ...
  return {
    ...
    permanent_id: {
      uri: CONDUCT_EXT_PERMANENT_ID,
      since: "0.4.3 (2026-09-09), section 12 of the markdown",
      registry: "perma-id/w3id.org#6653, merged 2026-09-09; the entry is public and versioned in git",
      resolves: "302 Found to " + u,
      identity: "the identifier of v1 is still " + u + ". This string is recognised when it is read and is written into the verdict as declared. It is not a second identifier.",
      accepted_uris: CONDUCT_EXT_URIS,
      exact: "the two strings above and nothing else: no case folding, no trailing slash, no http, no other path under w3id.org, no other version",
      writers: "declare the canonical string while v1 is current; a card that declares the permanent identifier is read and the verdict says which string it used",
      wire: "activation is accepted under either string and the echo returns the string the caller sent; metadata keys and Message.extensions always name the canonical string",
      never_resolved: "this gate does not follow the redirect while it checks: a fetch would let a third party's DNS decide what this extension is, and it would spend a request from the sweep's budget on an answer already known",
      succession: "if gate.horizonshield.dev stops resolving, " + CONDUCT_EXT_PERMANENT_ID + " is the address of record for the specification, and the next version of this extension MUST be identified by " + CONDUCT_EXT_NEXT_VERSION_ID,
      why: "A2A extension guidance: authors are encouraged to use a permanent identifier service, such as w3id.org, for their extension URIs to prevent broken links"
    },
    echo: {
      activate: "send the A2A service parameter A2A-Extensions containing this URI, or its permanent identifier " + CONDUCT_EXT_PERMANENT_ID + " (an HTTP header in the HTTP bindings); the 0.3 spelling X-A2A-Extensions is read as well",
      response_header: "A2A-Extensions carries back the string the caller sent (either accepted URI); when the request used X-A2A-Extensions the same value is echoed under that spelling too",
    ...
    walk: {
      ...
      assertions: ["card_bytes_stable", "conduct_ext_declared", "compensation_well_formed", "measured_endpoint_answered (null and not applicable when n3 answered 402: an endpoint that charges has answered, and a walk never pays)", "payment_required_as_declared (an endpoint answering 402 declares a paid model on its card; null when neither side of that applies)", "extension_echoed (null and not applicable when n3 was not an A2A message)"],
    ...
    how_this_gate_reads_it: "condition 3 (compensation_disclosure) reads the top-level compensation key and every capabilities.extensions[] entry carrying this URI or (0.4.3) its permanent identifier, and the verdict records which string was declared; one well-formed declaration passes, two that disagree fail, none fails. Nothing else in the params is judged by this gate.",
    ...
    versioning: "the URI ends in /v1; a breaking change uses a new URI; v1.1 (section 11) is an additive revision under the same URI; v1.2 (section 12, 2026-09-09) records that the w3id.org permanent identifier now resolves here and is recognised on read, that the identifier of v1 is unchanged, and that the next version MUST be " + CONDUCT_EXT_NEXT_VERSION_ID,
...
// 0.4.3: 綴りは 2 本とも受ける。echo は要求された綴りをそのまま返す(a2aEchoHeaders は activated をそのまま並べる)。
// 中身(metadata の鍵と Message.extensions)は常に正規の URI を名乗る。線は相手の言葉、データは 1 つの身元。
...
async function handleGateA2A(request, env, origin) {
  ...
  // 0.4.14. data part が witness_request なら証人応答 (measure -> 署名 -> observation)。鍵未設定なら正直に断る。
  const wreq = extractWitnessRequest(parts);
  if (wreq) {
    const wk = await loadWitnessKeyFromEnv(env);
    let wres;
    if (!wk) {
      wres = { answered: false, declined: "witness_not_configured", why: "this gate serves as a conduct witness but no witness signing key is configured here yet (WITNESS_PRIVKEY_B64 / WITNESS_PUBKEY_B64 unset), so it cannot sign an observation. This is not a refusal of the request; retry once the key is set." };
    } else {
      const verifyCard = async (card, org) => { const r = await verifyCardSignatures(card, org); const sg = (r && r.signatures && r.signatures[0]) || {}; return { verified: !!(r && r.verified === true), kid: sg.kid || "", alg: sg.alg || "", jku: sg.jku || "", why: (r && r.verified === true) ? "" : (sg.reason || (r && r.reason) || "not verified") }; };
      const measure = (target) => measureWitnessSurfaces(target, { fetchImpl: fetch, canonicalizeCard: cardSignatureCanonical, verifyCard });
      try {
        wres = await answerWitnessRequest(wreq, { signedDomain: new URL(CARD_CANONICAL_ORIGIN).host, keyUrl: CARD_CANONICAL_ORIGIN + "/keys/witness.json", priv: wk.priv, pubRaw: wk.pubRaw, measure });
      } catch (e) { return send({ error: { code: -32000, message: "witness_error: " + String(e && e.message || e) } }); }
    }
    let wr = witnessRpcResult(wres);
    if (activated.length) wr = a2aAttachConduct(wr, origin);
    return send({ result: a2aSendMessageResult(wr, wire) });
  }
  ...
  if (activated.length) result = a2aAttachConduct(result, origin);  // 0.4.3: どっちの綴りで活性化されても中身は同じ(付ける物は常に正規の URI を名乗る)
...
async function verifyCardSignatures(card, origin) {
  ...
  // 0.4.15. 素の RFC 8785 流(配られた card そのもの、signatures 抜き、射影なし)でも試す。野には 2 流儀ある:
  // A2A 1.0 §8.4 の proto 射影 + RFC 8785(公式 SDK)と、配信バイトへそのまま RFC 8785 を掛ける流儀(仕様を素直に読む検証器、sign_lib の plain)。
  // 以前は前者しか試さんかったので、2 本署名の card(扉自身と KIRA)を測ると plain の方を「署名後に書き換えられた」と記録しとった。事実と違う。
  const bareServed = Object.assign({}, card); delete bareServed.signatures;
  const plainCanonical = jcs(bareServed);
  const plain_canonical_sha256 = await sha256hex(plainCanonical);
  const plainPayloadB64 = bytesToB64url(new TextEncoder().encode(plainCanonical));
  const results = [];
  for (const sig of sigs.slice(0, 3)) {
    const r = { kid: null, jku: null, alg: null, verified: null, canonical: null, reason: null };
    try {
      ...
      // 0.4.5. 鍵の場所が相手のドメインの下かどうかは、alg や kid で落ちた行にも要る事実や。
      // 0.4.4 では https の検査の後にしか立てとらんかったので、落ちた行では undefined のままやった。
      try { r.jku_same_host = new URL(r.jku).host === new URL(origin).host; } catch (_e) { r.jku_same_host = null; }
      ...
      let ok = await crypto.subtle.verify({ name: "ECDSA", hash: "SHA-256" }, pub, sigBytes, new TextEncoder().encode(sig.protected + "." + payloadB64));
      if (ok) r.canonical = "a2a-1.0-proto";
      else {
        ok = await crypto.subtle.verify({ name: "ECDSA", hash: "SHA-256" }, pub, sigBytes, new TextEncoder().encode(sig.protected + "." + plainPayloadB64));
        if (ok) r.canonical = "rfc8785-served";
      }
      r.verified = !!ok;
      r.reason = r.canonical === "a2a-1.0-proto" ? "signature verifies over the RFC 8785 canonical card (schema fields only; fields outside the A2A 1.0 schema are not covered)"
        : r.canonical === "rfc8785-served" ? "signature verifies over RFC 8785 of the card exactly as served, signatures removed (every served field is covered, including fields outside the A2A 1.0 schema)"
        : "signature does not verify over either canonical form (the A2A 1.0 section 8.4 proto projection, or RFC 8785 of the card as served): the card was changed after signing, or was signed over some other form";
    } catch (e) {
  ...
  return { present: sigs.length, verified: anyTrue ? true : (anyFalse ? false : null), canonical_sha256, plain_canonical_sha256, signatures: results };
}
...
function checkCompensation(card) {
  ...
  const locations = ["compensation", "capabilities.extensions[uri=" + CONDUCT_EXT_URI + "].params.compensation", "capabilities.extensions[uri=" + CONDUCT_EXT_PERMANENT_ID + "].params.compensation (0.4.3, the same extension declared under its permanent identifier)"];
  const decls = [];
  ...
  for (const e of conductExtDeclarations(card)) {
    ...
    // 0.4.3. 正規の URI と永続識別子は同じ拡張。どっちで書かれたかは where と uri に残す。
    decls.push({ where: e.uri === CONDUCT_EXT_URI ? "extension" : "extension_permanent_id", uri: e.uri, c: p ? p.compensation : undefined });
  }
  const declaredUris = decls.filter((d) => d.uri).map((d) => d.uri);
  const extIdentifier = declaredUris.length ? { declared: declaredUris, forms: declaredUris.map(conductExtUriForm), identifier: CONDUCT_EXT_URI, note: declaredUris.every((u) => u === CONDUCT_EXT_URI) ? "declared under the canonical string" : "declared under the w3id.org permanent identifier, which 302 redirects to the canonical string; both are read as this extension since 0.4.3 and the identifier of v1 is unchanged. The redirect was not followed while checking." } : null;
  if (!decls.length) {
    return {
      pass: false,
      reason: "compensation block not declared in agent-card (neither location)",
      detail: { expected_shape, locations, extension_identifier: extIdentifier, accepted_extension_uris: CONDUCT_EXT_URIS }
    };
  }
  for (const d of decls) {
    if (!d.c || typeof d.c !== "object" || Array.isArray(d.c)) {
      return { pass: false, reason: "compensation block at " + d.where + " is not an object", detail: { expected_shape, locations, location: d.where, extension_identifier: extIdentifier } };
    }
  }
  const c = decls[0].c;
  for (const d of decls.slice(1)) {
    if (!compensationEqual(c, d.c)) {
      return { pass: false, reason: "two compensation declarations disagree (" + decls[0].where + " vs " + d.where + "): a disclosure that says two things is not a disclosure", detail: { locations, declared_at: decls.map((x) => x.where), extension_identifier: extIdentifier } };
    }
  }
  // 0.4.3. where は 3 種類になった(top_level / extension / extension_permanent_id)。
  // location の意味は今までどおり「top-level か、拡張か、両方か」。綴りの違いで both には落とさん。
  const family = (w) => (w === "top_level" ? "top_level" : "extension");
  const fams = [...new Set(decls.map((d) => family(d.where)))];
  const location = fams.length === 1 ? fams[0] : "both";
  ...
  const detail = {
    extension_identifier: extIdentifier,
    paid_by: c.paid_by,
...
function gateDisclaimers(record) {
  ...
  // 0.4.4 (2026-09-10). 署名は合否に効かん(0.3.4 のまま)。効くのは「この記録が何を証明するか」。
  // TLS は「取った瞬間」しか押さえん。署名が効くのは持ち出した時と時間が経った時 = 転送に耐える著者性。
  // せやから無署名の card についての行は、この扉の観測に帰属するだけで、運営者は否認できる。そう書く。
  // 0.4.5. 帰属は「verify したか」やなく「相手のドメインの下の鍵で verify したか」で決まる。
  // 他所のホストの鍵で verify しても、その鍵サーバが消えれば帰属は消え、運営者は否認できる。
  // 自ドメインの署名が落ちとる card は、他所の署名が通っても帰属させん。落ちた事実の方が重い。
  const sig = checks.agent_card && checks.agent_card.detail ? checks.agent_card.detail.signature : null;
  if (sig && typeof sig === "object") {
    const list = Array.isArray(sig.signatures) ? sig.signatures : [];
    const hostOf = (u) => { try { return new URL(String(u)).host; } catch (_e) { return "an unnamed key location"; } };
    const ownGood = list.find((x) => x && x.verified === true && x.jku_same_host === true) || null;
    const ownBad = list.find((x) => x && x.verified === false && x.jku_same_host === true) || null;
    const foreignGood = list.find((x) => x && x.verified === true && x.jku_same_host !== true) || null;
    const first = list.find((x) => x && x.verified === true) || list[0] || null;
    if (sig.present === 0) {
      dne.push("that the operator published this card: it carried no signature, so these bytes are attributable to this gate's observation alone and the operator can repudiate them (an unsigned card is permitted and is not a failure)");
    } else if (ownGood && !ownBad) {
      est.push("the agent card was signed with the key at " + (ownGood.jku || "an unnamed key location") + " (kid " + (ownGood.kid || "none") + "), which is served on the agent's own host, and the signature verifies over the canonical card, so what the card declares is attributable to the operator and not only to this gate's observation");
    } else if (ownBad) {
      dne.push("that the operator published this card: a signature whose key is served on the agent's own host (kid " + (ownBad.kid || "none") + ") does not verify (" + (ownBad.reason || "no reason recorded") + ")" + (foreignGood ? ", and the signature that does verify uses a key at " + hostOf(foreignGood.jku) + ", which is a different host from the card" : "") + ", so these bytes remain attributable to this gate's observation alone");
    } else if (foreignGood) {
      dne.push("that the operator published this card: the signature verifies, but against a key served at " + hostOf(foreignGood.jku) + ", which is a different host from the agent card. This gate checked host identity, not who controls each host, so it does not claim on this signature alone that the card is the operator's (a key on a different host is permitted and is not a failure)");
    } else {
      dne.push("that the operator published this card: a signature is present but this gate could not confirm it (" + ((first && first.reason) || sig.reason || "no reason recorded") + "), so these bytes remain attributable to this gate's observation alone");
    }
  }
  ...
  return {
    ...
    conditions: {
      ...
      compensation_disclosure: {
        location: "agent-card, top-level key 'compensation', or (0.3.2) the capabilities.extensions[] entry whose uri is " + CONDUCT_EXT_URI + " or (0.4.3) its w3id.org permanent identifier " + CONDUCT_EXT_PERMANENT_ID + ", key params.compensation. The verdict records which of the two strings the card declared, and the redirect is never followed while checking. One well-formed declaration passes. When both are present they must be equal on the five keys, or the condition fails.",
        shape: {
          paid_by: PAID_BY,
...
function openapiDoc(origin) {
  ...
  return {
    ...
    paths: {
      ...
      "/ext/legal-entity/v1": g("A2A Legal Entity Extension v1 (the URI is the identifier)", "JSON by default. Accept: text/markdown returns the specification text. Declared by agent cards under capabilities.extensions[]; puts the legal entity identifier inside the signed bytes."),
...
function ownAgentCard(origin) {
  return {
    ...
    url: origin + "/a2a",
    provider: { organization: "The HORIZ\u97f3s Co., Ltd.", url: "https://shield.the-horizons-innovation.com", legalEntity: Object.assign({}, LEGAL_ENTITY) },
    version: CONFIG.version,
    // 0.4.11 (2026-09-20). root の protocolVersion を 1.0 に。0.3 は supportedInterfaces[1] と url/preferredTransport に残るので
    // 0.3-only の読み手も /a2a を見つけられ、扉は method 名で両線を捌く(動きは 1 バイトも変わらん)。root を 0.3.0 に置いとった旧設計は
    // 後方互換の belt-and-suspenders やったが、外部 discovery(Agenstry)が root だけ読んで「pre-1.0 宣言」と減点しとった。A2A 1.0 は
    // 「最も新しい版を root に、古い版は supportedInterfaces に」が正しい形。
    protocolVersion: "1.0",
    ...
    capabilities: { streaming: false, pushNotifications: false, extensions: [conductExtension({
      ...
      identity: { kind: "did", ref: "did:web:gate.horizonshield.dev" },
      // 0.4.14. 扉は「引かれたら答える証人」でもある (TSUGI v2 籤の被証人側)。この宣言が池入りの条件の 1 つ。
      // reciprocal はここでは「籤で引かれたら (nenrin-witness-request-v1) 署名付き観測で答える」の意。11.6 の
      // 無勧誘の caller_card 歩き返しはまだやらん (参照実装 pending)。宣言と実装の差を残さんよう、下の witness_reply に
      // 扉が実際にやることだけを書く。鍵は運営者鍵とも card 鍵とも別 (env、/keys/witness.json が公開鍵を配る)。
      ...
      witness_reply: {
        ...
        key_url: origin + "/keys/witness.json",
        surfaces: WITNESS_SURFACES,
    ...
    skills: [
      {
        ...
        examples: ["Check https://example.com/mcp for conformance", "Does this server exist, publish an agent card, and return identical output for identical input?"],
      ...
      },
      {
        ...
        examples: ["Verify this verdict hashes to its own record_sha256", "Recompute the digest of a verdict returned by check"],
      ...
      },
      {
        ...
        tags: ["methodology", "transparency", "spec"],
        examples: ["What does this gate actually check?", "How is a verdict recomputed independently?"],
      ...
      },
      {
        ...
        tags: ["verification", "status", "honest-null"],
        examples: ["Is https://example.com/mcp currently verified?", "What is this endpoint's latest state on the register?"],
        inputModes: ["text/plain"],
        outputModes: ["application/json"]
      },
      {
        ...
        tags: ["register", "lookup", "recomputable"],
        examples: ["Look up the record for https://example.com/mcp", "Find the latest verdict hash for this endpoint"],
        inputModes: ["text/plain"],
        outputModes: ["application/json"]
      }
    ]
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // --- 運営者の許可鍵を配る口 (2026-09-20、0.4.8) ------------------------------------
    // TSUGI (継、旧称 Proof-of-Recovery) の Policy Gate。修復の許可 (nenrin-authorization-v1) は運営者の Ed25519 鍵で
    // 署名される。読む側は recovery_verify の strict モードにこの公開鍵を渡し、「署名の無い / 信用して
    // ない鍵 / 期限切れ」の許可の後に実行された修復を弾く。この URL は検証そのものには要らん (記録は
    // 鍵を署名バイトの中に持つ)。効くのは 1 点だけ: 「その鍵は運営者が自分のドメインで配っとる鍵か」。
    // agreement.json / witness.json と同じ規律。未設定なら 404 (「無い」)、空を 200 で返さん。
    // 秘密鍵はこの Worker に無い。署名は手元 (recovery-v0/authorize.mjs)。判定に一切関わらん。
    if (path === "/keys/operator.json") {
      const pub = (env.OPERATOR_PUBKEY_B64 || "").trim();
      if (!pub) {
        return json({
          error: "not_configured",
          what: "no recovery-authorization (operator) key is published for this domain yet",
      ...
      return new Response(JSON.stringify({
        ...
        schema: "nenrin-authorization-v1",
        what_this_is:
          "The Ed25519 public key this domain's operator signs recovery authorizations with (TSUGI, proof of recovery, " +
          "Policy Gate). A repair of a human-approval primitive is valid only if its nenrin-authorization-v1 record " +
          "is signed by this key, the key is in the reader's trust set, and the execution happened before expires_at. " +
          "Fetching it establishes one thing only: that this is the key this domain serves, so an authorization " +
          "signed by it is attributable to this operator.",
        what_this_is_not:
          "Proof that any particular repair was correct, or that the diagnosis it authorized was right. It is one public key. Verify chains against their own bytes.",
        spec: "https://github.com/ogasurfproject-jpg/horizon-shield/blob/main/workers/hs-ledger/nenrin/recovery-v0/README.md",
      }, null, 2), {
        status: 200,
    ...
    if (path === "/ext/legal-entity/v1" || path === "/ext/legal-entity/v1/") return await legalEntityExtResponse(request);
    ...
    return json({ error: "not_found", path, endpoints: ["/mcp", "/a2a", "/.well-known/agent-card.json", "/.well-known/jwks.json", "/check", "/is-verified", "/record/<record_sha256>", "/register/lookup", "/spec", "/ext/conduct/v1", "/ext/legal-entity/v1", "/self", "/history", "/changes", "/watchlist", "/watch", "/sweep", "/sweep/last", "/nenrin/window", "/health"] }, 404);
  }
};
