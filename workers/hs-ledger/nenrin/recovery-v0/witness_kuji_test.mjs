...
// ---- 4. request and intake, with a fake witness behind a fake fetch ----
{
  ...
  t("request: carries no expected values (blind: no expected key, no expected gate_commit or canonical hash in the bytes), names the 9 surfaces and the reply schema", !Object.keys(req).some((k) => k.startsWith("expected")) && !JSON.stringify(req).includes(records[6].expected_after["health.gate_commit"].gate_commit) && !JSON.stringify(req).includes(records[6].expected_after["agent-card.signature"].canonical_sha256) && req.surfaces.length === 9 && req.reply_schema === SCHEMAS.observation);
