...
設計書 (Claude Docs「Proof-of-Recovery 設計書」、2026-09-20) の v0。第2の柱の、記録の型と、それを採点する物と、Class 1 の証人。
...
- Recovery Agent (切り分けと提案を書く側)。今週は人 (番人) が書いた。手順は fixture の diagnosis にそのまま在る。
- drift_witness を worker の cron に。今は Mac から手で回す物。
- 前回 witness した状態との比較 (keys の有無、jwks の変化)。v0 は毎回の観測を書くだけで、prior state を持たん。
