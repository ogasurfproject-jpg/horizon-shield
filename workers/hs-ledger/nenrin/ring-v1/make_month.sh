...
# month's ring when one exists, then write rings/<month>.sha256 listing every ring's sha256 for anchoring.
set -eu
cd "$(dirname "$0")"
...
for h in history/*.json; do
  ...
  if [ -f "$prev" ]; then
    python3 make_ring.py --month "$m" --history "$h" --prev "$prev" --out rings
  else
    python3 make_ring.py --month "$m" --history "$h" --out rings
  fi
done
