...
async function hsHandleCompare(request, env) {
  try {
    var hsTk = request.headers.get("X-HS-TOKEN") || "";
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (pathname === "/canary" && request.method === "GET") {
        var __canaryEnc = new TextEncoder().encode(JSON.stringify(HS_AUDIT_CANARY_EX));
        var __canaryDigest = await crypto.subtle.digest("SHA-256", __canaryEnc);
        var __canaryHex = Array.from(new Uint8Array(__canaryDigest)).map(function(b) { return b.toString(16).padStart(2, "0"); }).join("");
        var __canaryLive = __canaryHex.slice(0, 16).toUpperCase();
        return new Response(JSON.stringify({
          purpose: "public witness for independent verification per JIDEC entries #3 (script) and #4 (bundled source)",
          canary_ex: HS_AUDIT_CANARY_EX,
          canary_expect_hash: HS_AUDIT_CANARY_EXPECT.hash,
          live_computed_hash: __canaryLive,
          match: __canaryLive === HS_AUDIT_CANARY_EXPECT.hash,
          verifier_hint: "Compare canary_ex to the CANARY in verify_finding_v0.py (JIDEC entry #3). live_computed_hash must equal canary_expect_hash and both must equal C025E288675EE898.",
          spec: "SPEC_HASH_INDEPENDENCE_v1.md (JIDEC entry #2)"
        }, null, 2), { headers: { "Content-Type": "application/json; charset=utf-8", ...corsHeaders() } });
      }
...
<div class="test">
  ...
  <div style="font-family: 'Liberation Sans'">\u3042\u3044\u3046\u3048\u304A\u65E5\u672C\u8A9E\u30C6\u30B9\u30C8\u6F22\u5B57\u30AB\u30BF\u30AB\u30CA ABC123</div>
      ...
      if (pathname === "/generate-and-send" && request.method === "POST") {
        const _tok = request.headers.get("X-PDF-Token") || "";
