...
- **第三波 配備完了(13:1x JST、端末出力で確認)**: 鍵 ~/.hs_card_key.pem 作成(241 bytes、repo 外)、5 card に署名(kid hs-2026-09、正規形 sha は公式 SDK の値と全一致: mcp 311cb0a9 / gate ebae77e2 / ledger d9cf967d / jidec 783af2de / femtech 8c98e665)、commit **849a3724**(20 ファイル)push、GATE_COMMIT=849a37242944(Version 2e5000b5)、hs-mcp / hs-ledger / hs-jidec-mcp deploy、femtech 3699208 push + deploy。本番: `verify.mjs https://<5 host>` 全部 `True hs-2026-09`(公式 verifier が本番の card を通した)。扉 /check に mcp → `verified 1 True hs-2026-09 True` = **扉が本番の card の署名を、公式 SDK と同じ正規形で読んで、有効と言うた**。残り: MCP registry publish(Actions、C16)→ claim_register 再実行(C03/C16)、仕様の錨(seed 済、append は決裁)、w3id PR、in-toto 8 輪(任意)、鍵マネージャに 1 行。

- **残り全部 完了(13:2x JST、TOshi の手、手順書 ops/a2a_remaining_20260906.md)**: (1) 仕様 v1 の錨 = **JIDEC entry 37**(claim 3aa5a50d、stamp rc=0、Bitcoin 確定待ち。確定後に #2211 へ 1 行追記、文は手順書 2d の下)。seed は 94af08f6 で repo に。(2) MCP registry publish #15 緑 → claim register **C09 PASS、C16 PASS(registry 0.3.4)、FAIL は C14(TWZRD)の 1 つだけ**。(3) w3id: fork ogasurfproject-jpg/w3id.org → `horizonshield/README.md` + `.htaccess` → **PR perma-id/w3id.org#6653**(4 commits、squash 依頼済み、merge は数日。merge 後 `curl -sI https://w3id.org/horizonshield/conduct/v1` で 302 を確認、仕様の文は変えん)。(4) in-toto: 8 輪の DSSE + rings/jwks.json = mcp-conduct-register 61663f0、README の段落 = b982013、verify 8/8 true。(5) 鍵マネージャ 13:45 版(card 署名鍵の行、秘密値は空欄)。
- **事故 2 件(同日)**: (a) 論文チャットと同じ作業木で同時に git を回して `.git/index.lock` に 2 回弾かれた → 掟: 同じ repo の git は一度に 1 チャット。(b) 鍵マネージャ 01:55 版(値入り)がチャットに添付され、LEDGER_ADMIN_TOKEN / YAKUMO_PUSH_TOKEN / ahs-org-push / CLOUDFLARE_API_TOKEN が露出(2 回目)。番人の線: LEDGER_ADMIN_TOKEN は回す(手順は 13:45 版のその行)、他 3 本は社長判断。掟: 鍵マネージャは値を入れたまま添付しない。
- **次 = 扉 0.3.5(論文チャットの発見 3 件 + 番人の 1 件、ops/gate_history_coordinate_20260906.md)**: A 履歴に coordinate_derivation を残す、C salt を block の前に(次窓の salt を前窓の掃引で先に作る、beacon は salt より後の block、GET /nenrin/window で commitment 公開)、B 基準高さ min(tip) - 6 の 1 つで hash 一致、D 窓ごとに規則を固定。判定規則は 0.3.0 のまま。red team と /spec の数(C03/C04)が動く。18:00Z の掃引前に配備できれば今夜 derived が初めて履歴に残る。
- **扉 0.3.5 作成済み(14:5x JST、番人、Mac の作業木に書いた、未 commit、手順 ops/gate_0_3_5_20260906.md)**: A 履歴に coordinate_derivation(summarise)、B 基準高さは quorum 番目の tip - 6 で hash 一致(源 3 つ、mempool.emzy.de 追加)、C 次の窓の salt を前の窓の掃引で先に作り beacon の block は header time が salt 以上に限る(fail closed)、D 窓ごとに規則を固定、GET /nenrin/window(commitment 即公開、閉じた窓の salt を reveal)、運営者専用 GET /nenrin/probe(x-sweep-token、KV に書かず試し引き)。suites: instant 40/40、sweep_coordinate 22/22(新)、gate 82/82、a2a_face、watch 29/29(cloud と Mac VM の両方)。**扉の card は version を含むので署名を打ち直す(sign.mjs)→ verify → commit → push → deploy_gate.sh → /nenrin/window と /nenrin/probe で確認 → 18:00Z の掃引で derived が履歴に初めて残る筋。** C16 は publish のやり直しが要る(server.json 0.3.5)。
- **有志の入口 + 募集の文 + 証人会(15:2x JST、番人、Mac の作業木、未 commit)**: (1) `workers/hs-ledger/nenrin/a2a-conduct-walk/` に `conduct_witness_mcp.py`(stdio MCP、tool 1 つ witness_walk、歩くのは呼び出し側の機械 = vantage は相手の物)、`pyproject.toml`(console scripts `a2a-conduct-walk` と `conduct-witness-mcp`、`uvx --from "git+https://github.com/ogasurfproject-jpg/horizon-shield#subdirectory=workers/hs-ledger/nenrin/a2a-conduct-walk" ...` で 1 コマンド、push 後に効く)、`README.md`(10 分で証人になる、英 + 日、正直な数 9 / 1 / 37)、`witness_mcp_selftest.py`(14/14、cloud と VM。stdout の flush 抜けを VM で見つけて直した)。`skills/conduct-witness/SKILL.md`(Claude Code 用、名前を発明せん、FAIL も記録)。(2) `ops/zenn_witness_post_20260906.md`(published: false の下書き)、`ops/show_hn_20260906.md`、`ops/agntcon_onepager_20260906.md`(9/10〜11 渋谷、廊下用の 1 枚)。(3) `ops/witness_council_design_20260906.md` v0.1: 輪の一覧と規則の版上げに証人 2 名以上の副署(ES256、各員の JWKS は各員の domain)、席は記録で決まる、運営者の票は quorum に数えん、DAO でも token でも無い、conduct-v1.1 の identity 受け口を初仕事に。決めること 4 つ(9 節)。順番: 扉 0.3.5 の配備 → 0.3.5 の commit と一緒にこれらも commit → Zenn 公開は TOshi の判断 → Federico への頼みは論文 2 v0.5 と hackathon 明け。
